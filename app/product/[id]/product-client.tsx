  ...
  return (
    <div className="space-y-6 lg:sticky lg:top-6">
      <div>
        ...
        <h1 className="font-serif text-4xl md:text-5xl font-bold text-neutral leading-tight mb-3">{product.name}</h1>
        <div className="flex items-center gap-4 mb-5">
          <span className="font-serif text-3xl md:text-4xl text-primary font-bold">£{product.price_gbp}</span>
        ...
        <p className="text-neutral/70 leading-relaxed max-w-xl">{product.description}</p>
      </div>
      ...
      {!showFitFinder && (
        ...
        >
          {suggestedSize ? `Size ${suggestedSize} recommended` : 'Need help finding your fit?'}
        </button>
      )}
      ...
      {showFitFinder && (
        <div className="bg-[#fbf6f9] border border-primary/10 p-5 rounded-2xl space-y-4">
          {fitFinderStep === 0 && (
            <>
              <h3 className="font-medium text-neutral">What&apos;s your body type?</h3>
              <div className="grid gap-2">
                {['Pear Shaped', 'Apple Shaped', 'Hourglass'].map((bodyType) => (
                  <button
                    key={bodyType}
          ...
          {fitFinderStep === 1 && (
            <>
              <h3 className="font-medium text-neutral">Select your usual size</h3>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <label className="block text-sm font-semibold tracking-[0.16em] uppercase text-neutral">Select size</label>
          <a href="#size-guide" className="text-xs text-primary underline underline-offset-4">
            Size Guide
          </a>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          onClick={() => (wishlistActive ? removeFromWishlist(product.id) : addToWishlist(product.id))}
      ...
      <div className="border-t border-neutral/10 pt-4 space-y-3">
        <details open className="group rounded-xl border border-neutral/10 bg-white px-4 py-3">
          ...
          <ul className="mt-4 space-y-2 text-sm leading-6 text-neutral/70">
            <li>Premium compression fabric for smooth shaping and a clean silhouette.</li>
            <li>Breathable, comfortable fit designed for everyday wear.</li>
            <li>Supportive structure that stays comfortable throughout the day.</li>
            <li>Available in multiple sizes and colors for a tailored fit.</li>
          </ul>
        </details>

        <details id="size-guide" className="group rounded-xl border border-neutral/10 bg-white px-4 py-3">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-sm font-semibold uppercase tracking-[0.16em] text-neutral">
            Shipping & Returns
            <ChevronDown size={16} className="transition group-open:rotate-180" />
          </summary>
          <p className="mt-4 text-sm leading-6 text-neutral/70">
            Free shipping on orders over £50. Returns accepted within 30 days for unworn items in original
            condition.
          </p>
        </details>
      </div>
      ...
      <div className="rounded-2xl bg-[#fbf6f9] p-4 text-sm text-neutral/70">
        ...
        <p>Use the fit finder above or message us for sizing advice before you place your order.</p>
      </div>
