    ...
    <main className="bg-white">
      ...
      <section className="bg-tertiary/40 py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <p className="text-xs uppercase tracking-[0.3em] text-primary">Saved Items</p>
          <h1 className="mt-3 font-serif text-4xl font-bold text-neutral">Wishlist</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-neutral/60">
            Keep track of the styles you love and move them into cart whenever you are ready to buy.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        {products.length === 0 ? (
          <div className="mx-auto max-w-xl rounded-3xl border border-neutral/10 bg-white p-10 text-center shadow-sm">
            ...
            <h2 className="font-serif text-2xl font-bold text-neutral">Your wishlist is empty</h2>
            <p className="mt-3 text-sm leading-6 text-neutral/60">
              Save products you like so you can come back to them later without signing in.
            </p>
            <Link
              href="/shop"
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.8fr)]">
            <div className="space-y-4">
              {products.map((product) => (
                ...
                >
                  <img src={product.images[0]} alt={product.name} className="h-32 w-32 rounded-2xl object-cover" />
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-primary">{product.category.replace('-', ' ')}</p>
                    <h2 className="mt-1 font-serif text-2xl font-bold text-neutral">{product.name}</h2>
                    <p className="mt-2 text-sm leading-6 text-neutral/60">{product.description}</p>
                    <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-neutral/70">
                      <span className="font-semibold text-primary">£{product.price_gbp}</span>
            ...
            <aside className="h-fit rounded-3xl border border-neutral/10 bg-[#fafafa] p-6 sticky top-28">
              <p className="text-xs uppercase tracking-[0.24em] text-neutral/50">Wishlist Summary</p>
              <h2 className="mt-2 font-serif text-2xl font-bold text-neutral">{products.length} items saved</h2>
              <p className="mt-3 text-sm leading-6 text-neutral/60">
                Guests can save items without account creation. Use this list to plan checkout later.
              </p>
              <button
