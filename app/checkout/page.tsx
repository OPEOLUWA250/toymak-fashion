...
import { CreditCard, MapPin, ShieldCheck, Truck, WalletCards } from 'lucide-react'
...
export default function CheckoutPage() {
  ...
  const selectedGateway = useMemo(() => getPaymentGatewayForCountry(country), [country])
  ...
  const tax = country.toLowerCase().includes('nigeria') ? subtotal * 0.075 : subtotal * 0.2
    ...
    return (
      <main className="bg-white">
        ...
        <section className="mx-auto max-w-4xl px-4 py-20 text-center sm:px-6 lg:px-8">
          <h1 className="font-serif text-4xl font-bold text-neutral">Checkout is empty</h1>
          <p className="mt-4 text-neutral/60">Add products to your cart before continuing to guest checkout.</p>
          <Link
            href="/shop"
    ...
    <main className="bg-white">
      ...
      <section className="bg-tertiary/40 py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <p className="text-xs uppercase tracking-[0.3em] text-primary">Guest Checkout</p>
          <h1 className="mt-3 font-serif text-4xl font-bold text-neutral">Fast, seamless checkout</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-neutral/60">
            No sign-in required. Your country automatically selects the correct payment gateway.
          </p>
        </div>
      </section>
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-neutral/10 bg-white p-6 shadow-sm">
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-serif text-2xl font-bold text-neutral">Shipping details</h2>
                  <p className="text-sm text-neutral/60">Enter your address to unlock the right payment option.</p>
                </div>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-neutral">Country</label>
                  <select
                    value={country}
                ...
                <Field label="Street address" value={street} onChange={setStreet} placeholder="Street address" />
                <Field label="City" value={city} onChange={setCity} placeholder="City" />
                <Field label="State / Region" value={stateRegion} onChange={setStateRegion} placeholder="State or region" />
                <Field label="Postal code" value={postalCode} onChange={setPostalCode} placeholder="Postal code" />
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-neutral/10 bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-serif text-2xl font-bold text-neutral">Payment method</h2>
                  <p className="text-sm text-neutral/60">
                    The selected method changes automatically based on the country above.
                  </p>
                </div>
              </div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="flex items-center gap-3">
                            <div className={`rounded-xl p-3 ${active ? 'bg-primary text-white' : 'bg-neutral-100 text-neutral'}`}>
                              <Icon size={18} />
                            </div>
                            <div>
                              <p className="font-semibold text-neutral">{option.title}</p>
                              <p className="text-xs text-neutral/60">{option.description}</p>
                            </div>
                          </div>
                        ...
                        <span className={`mt-1 rounded-full px-3 py-1 text-xs font-medium ${active ? 'bg-primary text-white' : 'bg-neutral-100 text-neutral/70'}`}>
          ...
          <aside className="space-y-6 sticky top-28 h-fit">
            <div className="rounded-3xl border border-neutral/10 bg-[#fafafa] p-6 shadow-sm">
              <div className="mb-5 flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-serif text-2xl font-bold text-neutral">Order summary</h2>
                  <p className="text-sm text-neutral/60">Review your items before payment.</p>
                </div>
              </div>
              ...
              <div className="space-y-4 border-b border-neutral/10 pb-5">
                {items.map((item) => (
                  <div key={`${item.product_id}-${item.size}-${item.color}`} className="flex gap-3">
                    <img src={item.image_url} alt={item.product_name} className="h-16 w-16 rounded-xl object-cover" />
                    ...
                    <p className="text-sm font-semibold text-neutral">£{(item.price_at_addition * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3 py-5 text-sm text-neutral">
                ...
                <Row label="Gateway" value={selectedGateway === 'paystack' ? 'Paystack' : 'Stripe'} accent />
              </div>
              ...
              <p className="mt-3 text-center text-xs text-neutral/50">
                Payment buttons will be wired in the next pass. The selected gateway is already determined.
              </p>
            </div>

            <div className="rounded-3xl border border-neutral/10 bg-white p-6 shadow-sm">
              <h3 className="font-serif text-xl font-bold text-neutral">Why this works</h3>
              <ul className="mt-4 space-y-3 text-sm leading-6 text-neutral/60">
                <li>• UK customers are mapped to Stripe automatically.</li>
                <li>• Nigerian customers are mapped to Paystack automatically.</li>
                <li>• The UI still shows both options so the flow is transparent.</li>
              </ul>
            </div>
          </aside>
        </div>
